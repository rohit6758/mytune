      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          ...
          <button 
            ...
            className="flex items-center gap-1 text-sm text-[#D0FF00] font-bold bg-[#D0FF00]/10 px-3 py-1.5 rounded-full hover:bg-[#D0FF00]/20 transition-colors"
          >
        ...
        {isCreating && (
          <div className="flex gap-2 mb-4 bg-[#121212] p-3 rounded-xl border border-white/10">
            ...
            />
            <button onClick={createPlaylist} className="bg-[#8B16FF] text-white px-4 py-1.5 rounded-lg font-bold hover:bg-[#7200e6]">Create</button>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {likedTracks.length > 0 && (
            <button 
              ...
              className="w-10 h-10 rounded-full bg-[#D0FF00] text-black flex items-center justify-center hover:scale-105 transition-transform shadow-[0_4px_14px_0_rgba(208,255,0,0.2)]"
            >
            ...
            {likedTracks.map((track, index) => {
              ...
              return (
                <div 
                  ...
                  className={`flex items-center gap-3 p-2 rounded-xl cursor-pointer transition-colors group ${isPlaying ? 'bg-[#D0FF00]/10' : 'hover:bg-white/5'}`}
                >
                    ...
                    {isPlaying && (
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                        <div className="w-3 h-3 flex justify-between items-end">
                          <div className="w-[3px] bg-[#D0FF00] h-full animate-bounce" style={{ animationDelay: '0ms' }} />
                          <div className="w-[3px] bg-[#D0FF00] h-2/3 animate-bounce" style={{ animationDelay: '150ms' }} />
                          <div className="w-[3px] bg-[#D0FF00] h-full animate-bounce" style={{ animationDelay: '300ms' }} />
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-[15px] font-bold truncate ${isPlaying ? 'text-[#D0FF00]' : 'text-white'}`}>{track.title}</p>
                    <p className="text-xs text-white/50 truncate mt-0.5">{track.artist}</p>
                  </div>
                  
                  <button 
                    onClick={(e) => removeTrack(track.track_id, e)}
                    className="p-2 opacity-0 group-hover:opacity-100 transition-opacity text-[#D0FF00]"
                  >
