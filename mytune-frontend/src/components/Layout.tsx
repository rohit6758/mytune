...
export default function Layout() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-on-background w-full">
      ...
      <nav className={clsx(
        "md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center py-3 border-t",
        isDiscover 
          ? "bg-black/80 backdrop-blur-xl border-white/10" 
          : "bg-black/95 backdrop-blur-lg border-surface-container/50 pb-[env(safe-area-inset-bottom)]"
      )}>
        {navItems.map(item => {
           ...
           if (item.name === 'Create' && !isDiscover) {
             return (
               <NavLink key={item.name} to={item.path} className={({ isActive }) => clsx(
                 "flex flex-col items-center justify-center -mt-8 rounded-full w-14 h-14 shadow-lg transition-transform hover:scale-105 duration-300",
                 isActive ? "bg-gradient-to-br from-[#ff5540] to-[#ff9900] text-black shadow-[#ff5540]/30" : "bg-gradient-to-br from-[#ff5540] to-[#ff9900] text-white"
               )}>
           ...
           return (
             <NavLink key={item.name} to={item.path} className={({ isActive }) => clsx(
               "flex flex-col items-center justify-center p-2 group transition-colors flex-1",
               isActive 
                 ? "text-[#ff5540]" 
               ...
               {({ isActive }) => (
                 <>
                   <span className="material-symbols-outlined mb-1 transition-transform" style={{ fontVariationSettings: isActive ? "'FILL' 1" : "'FILL' 0", fontSize: '28px' }}>
                     {item.icon}
                   </span>
